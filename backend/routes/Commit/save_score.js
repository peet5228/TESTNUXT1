...
router.get('/user/:id_eva',verifyToken,requireRole('กรรมการประเมิน'),async (req,res) => {
    try{
        ...
        const [rows] = await db.query(`select * from tb_member m,tb_eva e,tb_system s,tb_commit c where c.id_member=? and c.id_eva=? and e.status_eva=? and c.id_eva=e.id_eva and e.id_member=m.id_member and e.id_sys=s.id_sys order by e.id_eva desc`,
            [id_member,id_eva,2]
        )
        res.json(rows[0])
    }catch(err){
...
router.post('/save/:id_eva',verifyToken,requireRole('กรรมการประเมิน'),async (req,res) => {
    try{
        ...
        const detail_commit = req.body.scores
        const id_eva = req.params.id_eva
        ...
        if(Commit.level_commit === 'ประธาน'){
            COMMITS = 2
        }else if(Commit.level_commit === 'ประธาน'){
            COMMITS = 3
        }else if(Commit.level_commit === 'ประธาน'){
            COMMITS = 4
        }
        for(const item of scores){
            await db.query(
                ...
                [id_eva,item.id_indicate,COMMITS,item.score]
            )
        }
        ...
        const [CommitTwo] = await db.query(`select * from tb_commit where id_eva=? and status_commit=?`,[id_eva,'y'])
        if(CommitTwo.length === 3){

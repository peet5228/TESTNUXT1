        ...
        <c-row>
            <v-col cols="12">
                <v-form v-if="user.status_eva === 2 || user.status_eva === 3">
                    <v-h1 class="text-h5 font-weight-bold">คะแนนประเมินกรรมการประเมิน</v-h1>
                        ...
                        <v-col cols="12">
                            ...
                            <v-table class="table">
                                ...
                                <tr v-for="indicate in topic.indicates" :key="indicate.id_indicate">
                                    ...
                                    <td class="border pa-1 text-center">{{ indicate.score_commit*indicate.point_indicate }}</td>
                                </tr>
                            </v-table>
                        </v-col>
                    </v-row>
                    <div class="mt-4 text-end">
                        <v-card class="pa-4" color="success">คะแนนรวมทั้งหมด : {{ totalScore }} คะแนน</v-card>
                    </div>
                </v-form>
...
const fetchTopic = async () =>{
    try{
        const token = localStorage.getItem('token')
        const res = await axios.get(`${commit}/score_commit/indicate/${id_eva}`,{headers:{Authorization:`Bearer ${token}`}})

                ...
                <v-card>
                    ...
                    <v-card-text>
                        <v-form @submit.prevent="saveMember">
                            <v-row>
                                <v-col cols="12" md="6">
                                    <v-select label="หัวข้อการประเมิน" v-model="form.id_topic" :items="topic.map( p => ({title:p.name_topic,value:p.id_topic}))" :error-messages="error.id_indicate" />
                                </v-col>
                                <v-col cols="12" md="6">
                                    <v-text-field label="ตัวชี้วัด" v-model="form.name_indicate" :error-messages="error.name_indicate" />
                                </v-col>
                                <v-col cols="12" md="12">
                                    <v-textarea label="รายละเอียด" v-model="form.detail_indicate" :error-messages="error.detail_indicate" />
                                </v-col>
                                <v-col cols="12" md="6">
                                    <v-select label="น้ำหนักคะะแนน" v-model="form.point_indicate" :items="[1,2,3,4,5]" :error-messages="error.point_indicate" />
                                </v-col>
                                <v-col cols="12" md="6">
                        ...
                        <v-table>
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-center border">นำหนักคะแนน</th>
                            ...
                            <tbody>
                                <tr v-for="(items,index) in result" :key="items.id_topic">
                                    ...
                                    <td class="text-center border">{{ items.name_indicate }}<br><span class="opacity-75">{{ items.detail_indicate }}</span></td>
...
function validateForm(){
    error.value = {}
    const f = form.value
    if(!f.id_topic)error.value.id_topic='กรุณากรอกเลือกหัวข้อการประเมิน'

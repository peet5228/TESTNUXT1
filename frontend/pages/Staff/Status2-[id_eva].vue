                            ...
                            <tbody>
                                <tr v-for="(items,index) in result" :key="items.id_eva">
                                    ...
                                    <td class="text-center border">
                                        <v-btn :color="bg(items.status_commit)" size="small" class="text-white">{{ items.status_commit === 'n' ? 'ยังไม่ได้ประเมิน' : 'ประเมินแล้ว' }}</v-btn>
                                    </td>
                                </tr>
...
const bg = (status_commit:string) => {
    ...
    // else if(status_commit === 2) return 'warning'
